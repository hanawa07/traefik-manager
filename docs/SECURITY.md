...
- 감사 로그 CSV는 현재 조회 권한과 필터를 그대로 사용하며, 스프레드시트 수식 시작 문자를 이스케이프합니다.
- 오래된 감사 로그 아카이브는 backend 데이터 볼륨 안에 gzip JSONL로 기록하고 `0600` 권한을 적용합니다. 아카이브 비활성화 시 보존 기간이 지난 로그는 복구할 수 없습니다.
- 보안 알림 실제 전송 성공/실패도 `alert` 감사 이벤트로 남기고, 설정 화면에서 최근 전송 이력/최근 24시간 실패 수를 바로 확인할 수 있습니다.
- 실패한 전송은 감사 로그 화면과 설정 화면에서 현재 채널 설정으로 즉시 재시도할 수 있습니다.
...
- `settings_update_*`, `service/redirect/middleware/user`의 `create/update/delete`, 각종 `*_rollback` 이벤트도 `운영 변경 알림` 정책으로 기본 채널 또는 override를 탈 수 있습니다.
- 인증서 상태 점검은 `certificate_warning`, `certificate_error`, `certificate_recovered` 감사 이벤트를 만들고, 같은 인증서가 같은 상태에 머무는 동안에는 중복 알림을 다시 보내지 않습니다.
- 인증서 화면에는 현재 경고 상태의 `중복 경고 억제 중` 여부와 상태 시작 시각이 표시됩니다.
...
| 항목 | 상태 | 적용 내용 |
|------|------|-----------|
| 관리자 알림/차단 정책 예외 | 완료 | 신뢰 네트워크 예외, 이벤트별 알림 비활성화 및 채널 override |
| 추가 로그인 검증 | 완료 | Cloudflare Turnstile `always`/`risk_based` 모드 |
| 로그인 보안 채널 확장 | 완료 | Teams, Email, PagerDuty 포함 provider preset |

추가 예외 정책은 실제 오탐이나 운영 차단 사례가 발생할 때 검토합니다.
