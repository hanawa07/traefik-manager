...
- user systemd 설치기는 자신의 timer/service가 정상임을 확인한 뒤 `--refresh-baseline`에 해당 unit만 명시합니다. 이 제한 갱신은 다른 unit의 추가·삭제·내용 변경이나 실패 상태를 발견하면 기존 기준선을 유지한 채 실패합니다. 설치기는 변경 전에 기존 main unit 파일·심볼릭 링크와 timer/path의 enable·active 상태를 임시 저장하며 파일 설치, `daemon-reload`, enable/start, 상태 검증, 기준선 갱신 중 하나라도 실패하면 종료 trap에서 원상 복구합니다. 기준선 갱신 성공을 설치 트랜잭션의 커밋 지점으로 사용합니다. 수동 변경도 `scripts/user-systemd-unit-watchdog.sh --refresh-baseline <timer> <service>`로 승인하며, 폐기할 timer/service를 먼저 비활성화한 뒤 `--retire-baseline <timer> <service>`로 현재 발견 목록에서 사라진 해당 unit만 제거합니다. 폐기 대상이 아직 활성 상태이거나 다른 unit이 함께 추가·삭제·변경되면 기존 기준선을 유지합니다. 전체 `--write-baseline`은 기준선 최초 생성에만 사용합니다. 관련 self-test는 임시 홈의 가짜 systemd와 가짜 알림만 사용하므로 실제 운영 알림을 보내지 않습니다.
