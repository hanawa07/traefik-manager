...
- 대시보드 Manager 배포 카드는 Docker 상태를 30초마다 갱신하며, `unhealthy`이면 연속 실패 횟수와 마지막 검사 시각·종료 코드를 표시합니다. 외부 watchdog 상태·연속 실패·마지막 실행과 최근 알림 워크플로 요청 결과도 표시하고, 설정한 지연 판정 시간이 지나면 상단 경고를 노출하며 healthcheck 원문 출력은 노출하지 않습니다.
...
- 대시보드의 `Docker 상태 전이 이력`은 최근 Manager 이상·복구 감사 기록을 30초마다 갱신합니다. 감사 로그는 `Manager 전체`, `Manager 이상`, `Manager 복구` 필터를 제공하며 이상·복구 필터에는 최근 100건 기준 건수를 표시합니다.
