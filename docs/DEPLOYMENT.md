...
- backend는 자체 `/api/health`, frontend는 backend까지 이어지는 `/api/health`를 Docker healthcheck로 사용하므로 `docker compose ps`에서 둘 다 `healthy`인지 확인합니다. frontend는 backend가 `healthy`가 된 뒤 시작하며, 시작 유예 구간에는 두 컨테이너를 2초마다 확인해 Traefik 라우터 등록 공백을 줄입니다.
...
- 운영 로그인·화면 스모크는 대시보드의 `Docker 정상`, Manager API 오류 추이 24개 기본 구간과 6시간·경로 필터 전환·임계치 감지 상태·영속 로그 회전 상태·권장값 API, 감사 로그 조건 CSV, 설정 화면의 `Artifact 만료`와 감사 로그 보존 카드를 명시적으로 확인합니다. Manager API 감사 자동 펼침은 운영 감사 데이터를 만들지 않고 브라우저 요청에 fixture 한 건을 주입해 매번 검사합니다. 관리자 세션에서는 저장 없이 권장값 계산·제외 경로 미리보기·입력 적용도 확인합니다.
