...
- 대시보드의 `Manager 상태 전이 이력`은 Docker 및 외부 watchdog의 최근 이상·복구 감사 기록을 30초마다 갱신합니다. watchdog 실행이 설정 기준보다 늦거나 다시 정상 갱신되면 각각 감사 로그에 기록합니다. 감사 로그는 행위자·대상 이름·대상 ID 검색과 `Manager 소스`·`Manager 상태` 조합, 반대 축 기준 교차 집계 수치를 지원합니다. 전체·24시간·7일·30일·90일 기간과 페이지당 25·50·100건을 선택할 수 있고 페이지 번호를 직접 입력해 이동할 수 있습니다. 적용 조건은 개별 제거하거나 전체 초기화할 수 있으며 필터 변경 중에도 화면을 닫지 않고 표만 갱신하고, 모바일에서는 필터 필드를 한 열로 배치합니다. 검색어와 선택한 필터·기간·Manager 소스·Manager 상태·전송 상태·채널·집계 기간·페이지 크기·페이지는 URL에 저장되어 새로고침 후에도 유지됩니다.
