...
- 대시보드 Manager 배포 카드는 Docker 상태를 30초마다 갱신하며, `unhealthy`이면 연속 실패 횟수와 마지막 검사 시각·종료 코드를 표시합니다. 외부 watchdog 상태·연속 실패·마지막 실행과 최근 알림 워크플로 요청 결과·실행 링크·최근 실행 5건의 최종 상태도 표시하고, 설정한 지연 판정 시간이 지나면 상단 경고를 노출하며 healthcheck 원문 출력은 노출하지 않습니다.
...
- 대시보드의 `Manager 상태 전이 이력`은 Docker 및 외부 watchdog의 최근 이상·복구 감사 기록을 30초마다 갱신합니다. watchdog 실행이 설정 기준보다 늦거나 다시 정상 갱신되면 각각 감사 로그에 기록합니다. 감사 로그는 `Manager 소스`와 `Manager 상태`를 동시에 조합할 수 있고, 필터 변경 중에도 화면을 닫지 않고 표만 갱신합니다. 선택한 필터·Manager 소스·Manager 상태·전송 상태·채널·집계 기간은 URL에 저장되어 새로고침 후에도 유지됩니다.
