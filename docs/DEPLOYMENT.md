...
- 성공 배포의 가장 느린 단계가 기본 60초를 초과한 상태로 3회 연속 이어지면 `host-operation-alert.yml`을 한 번 요청합니다. 같은 연속 구간에서는 중복 알림을 보내지 않고 정상 또는 실패 기록이 나오면 사건 상태를 해제합니다. 기준은 `TM_DEPLOY_BOTTLENECK_ALERT_THRESHOLD_MS`, 횟수는 `TM_DEPLOY_BOTTLENECK_ALERT_CONSECUTIVE`로 조정합니다.
