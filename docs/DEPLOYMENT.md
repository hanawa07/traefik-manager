...
- 전용 timer 자체가 꺼진 경우를 놓치지 않도록 Manager health watchdog도 같은 감시 스크립트를 교차 실행합니다. 상태·기준선·로그는 `~/.local/state/traefik-manager/user-systemd-unit-watchdog.*`와 `user-systemd-unit-baseline.sha256`에 유지되며 알림에는 unit 이름과 정형 원인만 포함합니다.
- 의도적으로 user timer/service를 추가하거나 unit·drop-in을 바꾼 뒤에는 먼저 모든 대상이 정상인지 확인하고 설치기를 다시 실행해 기준선을 갱신합니다. `scripts/test-user-systemd-unit-watchdog.sh`와 `scripts/test-install-user-systemd-unit-watchdog-timer.sh`는 임시 홈의 가짜 systemd와 가짜 알림만 사용하므로 실제 운영 알림을 보내지 않습니다.
