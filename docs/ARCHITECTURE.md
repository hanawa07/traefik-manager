...
- 감사 로그 조회와 CSV 내보내기는 같은 SQL 필터 빌더를 공유합니다. 보존 작업은 기본 365일을 기준으로 오래된 행을 배치 처리하며, 삭제 전 gzip JSONL 아카이브를 선택할 수 있습니다.
- Manager API 404·5xx 추이는 별도 이벤트 저장소를 만들지 않고 Docker Socket으로 backend 구조화 요청 로그를 읽어 요청 시점에 최근 24시간을 집계합니다.
