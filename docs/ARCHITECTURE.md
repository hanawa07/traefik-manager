...
- Manager API 404·5xx 추이는 별도 이벤트 저장소를 만들지 않고 Docker Socket으로 backend 구조화 요청 로그를 읽어 요청 시점에 6·12·24시간과 경로 조건을 집계합니다. 별도 15분 기본 상태 머신은 설정된 임계치 초과·복구를 감사 로그와 기존 `manager_health` 운영 알림 경로에 기록합니다.
