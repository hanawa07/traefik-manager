...
- 감사 로그 조회와 CSV 내보내기는 같은 SQL 필터 빌더를 공유합니다. 보존 작업은 기본 365일을 기준으로 오래된 행을 배치 처리하며, 삭제 전 gzip JSONL 아카이브를 선택할 수 있습니다. 관리자 복원은 파일 전체를 선검증하고 기존 감사 UUID를 덮어쓰지 않습니다.
- Manager API 404·5xx 추이는 `backend-data` 볼륨의 크기 제한 회전 JSONL을 읽어 요청 시점에 6·12·24시간과 경로 조건을 집계하며, 영속 로그가 없거나 열리지 않으면 Docker Socket의 backend 로그로 폴백합니다. 읽기 전용 preview API는 저장 전 제외 목록을 적용한 뒤 최근 24시간의 임의 집계 구간 최고치에 20% 여유를 더하고 기존 기본값보다 낮지 않은 권장 임계치를 반환합니다. 중첩 제외 경로의 오류는 가장 구체적인 접두어에 한 번만 집계하며 경로별 최근 오류 시각도 같은 로그 순회에서 계산합니다. 별도 15분 기본 오류 상태 머신은 설정된 임계치 초과·복구를, 30초 보관 상태 머신은 영속 로그 80% 사용·Docker 폴백·사용 불가와 복구를 감사 로그 및 기존 `manager_health` 운영 알림 경로에 기록합니다. 마지막 오류 점검 결과는 기존 시스템 설정 상태값으로 대시보드와 상단 경고에 제공합니다. `/api/` 접두어 제외 목록은 임계치 판정에만 적용해 추이 원본은 유지합니다.
