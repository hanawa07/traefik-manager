...
- 두 frontend는 `proxy_net`에 고유 alias로 연결한다.
...
- active slot과 배포 revision은 별도 상태 파일에 기록한다.
- inactive backend는 background task를 실행하지 않는다.
...
3. inactive backend와 frontend를 background task 비활성 상태로 시작한다.
4. DB migration 사전 검증과 후보 `/api/health`를 수행한다.
5. 후보 frontend를 내부에서 연속 3회 확인한다.
...
9. 새 active backend의 background task를 활성화한다.
...
- 전환 상태 파일에는 slot과 revision만 저장하고 비밀값은 기록하지 않는다.

## 구현 단위

1. background task를 API process 시작과 분리하고 inactive 모드를 추가한다.
2. compose에 blue/green 고유 service와 network alias를 추가한다.
3. active slot 읽기, 후보 준비, 원자 전환, rollback을 수행하는 호스트 스크립트를 추가한다.
4. Docker 상태 조회와 대시보드 배포 카드가 active slot을 표시하도록 확장한다.
5. 기존 단일 교체 명령은 비상 fallback으로 유지한다.
