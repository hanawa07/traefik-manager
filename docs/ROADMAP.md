...
- [ ] 로그인 페이지
- [ ] 대시보드 (서비스 현황 카드, 총 서비스 수, 인증 활성 수, 인증서 만료 임박)
- [ ] 서비스 목록 (도메인, 업스트림, TLS 상태, 인증 상태)
- [ ] 서비스 추가 폼 (도메인, 업스트림 호스트/포트, TLS, 인증 토글)
- [ ] 서비스 수정
- [ ] 서비스 삭제 (확인 모달)
- [ ] 인증 토글 → Authentik Provider/Application 자동 생성/삭제
...
- [ ] 인증서 목록 (Traefik API 연동, 도메인별 만료일 표시)
- [ ] 인증서 만료 임박 알림 (대시보드)
- [ ] HTTP → HTTPS 자동 리다이렉트 미들웨어
- [ ] IP 허용/차단 미들웨어 (서비스별)
- [ ] Authentik 접근 그룹 설정 (서비스별로 다른 그룹 지정)
