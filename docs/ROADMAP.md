...
- 관리자 인증, 업스트림 정책, 헬스 체크, 보안 알림까지 핵심 아키텍처는 고정됐다.
- 현재 단계는 설계 미완이 아니라 운영 안정화, 선택 기능 추가, 채널 확장 같은 유지보수/확장 단계다.
- 이후 작업은 구조를 다시 짜는 일이 아니라, 이미 확정된 경계 안에서 기능을 보강하는 성격으로 다룬다.
...
- [x] Traefik 디버그 대시보드 public route on/off
...
- [x] 보안 이벤트 외부 웹훅 알림
- [x] Slack/Discord/Telegram/Teams/PagerDuty/Email 보안 알림 preset
- [x] 선택형 Cloudflare Turnstile 로그인 보호
- [x] Turnstile 모드 확장 (항상 적용 / 위험 기반 적용)
- [x] 설정 변경 diff 표시 + 안전 롤백 (`시간 표시`, `업스트림 보안`)
- [x] 서비스/리다이렉트 변경 diff 표시 + 안전 롤백
- [x] 미들웨어/사용자 변경 diff 표시 + 안전 롤백
...
- [x] Basic Auth 미들웨어 (bcrypt htpasswd 해시, Authentik과 상호 배타, {router}-basicauth 생성)
...
- [x] Cloudflare DNS 연동 (서비스 생성/수정 시 upsert, 실패 시 롤백, 설정 화면 멀티존 드리프트 진단/일괄 재동기화/고아 레코드 정리, 비Cloudflare 도메인 자동 제외)
...
- [x] 알림 전송 이력/실패 가시화 + delivery 실패 재시도
- [x] 인증서 만료/만료 임박 알림 + 중복 억제
- [x] 대시보드 운영 경고 요약 (현재 인증서 상태 + 최근 인증서 경고 전환)
- [x] 인증서 경고 수동 재검사 액션
- [x] 인증서 경고 억제/복구 이력 가시화
- [x] 인증서 프리플라이트 반복 실패 감지 + 알림
- [x] 인증서 프리플라이트 자동 재검사
- [x] 인증서 프리플라이트 반복 실패 알림 쿨다운
- [x] 인증서 진단 기준 설정 UI + runtime override
- [x] 인증서 상태 전이 / 발급 반복 실패 알림 정책 분리
- [x] 감사 로그 알림 전송 성공/실패/채널 필터
- [x] 감사 로그 현재 조건 CSV 내보내기
- [x] 감사 로그 보존 기간·gzip 아카이브 정책
- [x] Manager API 404·5xx 24시간 추이
- [x] 감사 로그 아카이브 다운로드·중복 안전 복원
- [x] Manager API 오류 추이 기간·경로 필터, 임계치 알림·제외 경로·24시간 권장값·점검 상태 및 상세 전이 이력
- [x] 선택형 Cloudflare Turnstile 로그인 보호
- [x] 차단 이벤트 대응 자동화 고도화
...
즉, 현재 기준으로는 "다음 설계 단계"보다 "유지보수 및 기능 확장 단계"로 보는 편이 적절합니다.
