...
import { checkCertificateDrawer, checkMobileSidebar, checkOptionalAdminModal } from "./dashboard-visual-interactions.mjs";
...
export async function runDashboardVisualSmoke({ artifactDir, baseUrl, capabilities, cdp, timeoutMs }) {
  ...
  await checkListRequestRecovery({ baseUrl, cdp, timeoutMs });
  labels.push("서비스·미들웨어·리다이렉트·인증서 API 404 표시·재시도 복구");
