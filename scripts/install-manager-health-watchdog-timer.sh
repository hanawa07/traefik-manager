...
write_service_unit() {
  ...
  {
    printf '%s\n' '[Unit]'
    printf '%s\n' 'Description=Check Traefik Manager health and dispatch incident alerts'
    printf 'ConditionFileIsExecutable=%s/manager-health-watchdog.sh\n' "${SCRIPT_DIR}"
    printf '%s\n' '' '[Service]'
    printf '%s\n' 'Type=oneshot'
    printf 'WorkingDirectory=%s\n' "${REPO_ROOT}"
    printf 'Environment=HOME=%s\n' "${HOME}"
    printf '%s\n' 'Environment=PATH=/usr/local/bin:/usr/bin:/bin'
    printf 'Environment=TM_MANAGER_WATCHDOG_STATE_DIR=%s\n' "${STATE_DIR}"
    printf 'ExecStart=/usr/bin/bash %s/manager-health-watchdog.sh\n' "${SCRIPT_DIR}"
    printf '%s\n' 'UMask=0022'
...
write_timer_unit() {
  ...
  {
    printf '%s\n' '[Unit]'
    printf '%s\n' 'Description=Check Traefik Manager health every five minutes'
    printf '%s\n' '' '[Timer]'
    printf '%s\n' 'OnActiveSec=3min'
...
remove_legacy_cron_block() {
  ...
  if [[ "${begin_count}" != "1" || "${end_count}" != "1" ]]; then
    echo "기존 Manager health watchdog cron 마커 쌍이 올바르지 않습니다" >&2
    return 1
  fi
