...
export async function checkAuditRotationExportRecovery({ cdp, timeoutMs }) {
  ...
  if (expectedLatestFailure) {
    await waitForCondition(
      ...
      `Boolean(document.querySelector('[data-testid="secret-rotation-export-latest-failure"]')) &&
        Boolean(document.querySelector('[data-testid="secret-rotation-export-latest-failure-actions"]')) &&
      ...
      timeoutMs,
      "최근 Secret 회전 실패 단계와 CSV 대상 건수를 불러오지 못했습니다",
    );
  }
  await waitForCondition(
    cdp,
  ...
  const latestRecovery = await evaluate(cdp, `(async () => {
    ...
    const failureActions = document.querySelector('[data-testid="secret-rotation-export-latest-failure-actions"]');
    ...
    const latest = response.ok ? (await response.json())[0] : null;
    const failureCsvResponse = failureCsv?.href ? await fetch(failureCsv.href) : null;
    const failureCountResponse = failureDate
      ? await fetch('/api/v1/audit?event=smoke_rotation_failed&start_date=' +
          encodeURIComponent(failureDate) + '&end_date=' + encodeURIComponent(failureDate) +
          '&limit=1&offset=0')
      : null;
  ...
  if (expectedLatestFailure) {
    ...
    assert.match(latestRecovery.latestFailureCsvText, /실패 날짜 CSV/, "최근 회전 실패 날짜 CSV 문구가 없습니다");
...
export async function assertRotationCount({ cdp, timeoutMs }) {
  await waitForCondition(
    cdp,
