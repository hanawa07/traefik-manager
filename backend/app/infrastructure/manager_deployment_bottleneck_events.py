...
def read_manager_deployment_bottleneck_events(
    ...
    return _read_normalized_events(_manager_deployment_bottleneck_events_path(path))[
        :MAX_EVENTS
    ]
...
def preview_manager_deployment_bottleneck_event_cleanup(
    ...
    *,
    ...
    lines = _read_event_lines(
        _manager_deployment_bottleneck_events_path(path),
        strict=True,
    )
...
def _event_storage_summary(events: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "oldest_event_at": (
            min(occurred_values, key=_event_timestamp) if occurred_values else None
        ),
        "newest_event_at": (
            max(occurred_values, key=_event_timestamp) if occurred_values else None
        ),
    }
