...
async def _get_user_update_audit_log(db: AsyncSession, audit_log_id: str):
    ...
    if audit_log.resource_type != "user" or audit_log.action != "update":
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="사용자 변경 로그만 롤백할 수 있습니다")
    return audit_log
...
def _get_supported_user_rollback_payload(audit_log) -> dict:
    ...
    if (
        ...
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="이 사용자 변경은 안전 롤백을 지원하지 않습니다")
