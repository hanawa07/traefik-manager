...
def build_manager_health_summary(
    *,
    ...
    return AuditManagerHealthSummaryResponse(
        ...
        api_unhealthy_count=sum(
            event in MANAGER_SOURCE_EVENTS["api"] & MANAGER_UNHEALTHY_EVENTS
            for event in events
        ),
        api_recovered_count=sum(
            event in MANAGER_SOURCE_EVENTS["api"] & MANAGER_RECOVERED_EVENTS
            for event in events
        ),
