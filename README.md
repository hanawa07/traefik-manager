...
| 운영 | Docker 컨테이너 자동 감지, Cloudflare DNS 설정 UI, 시간 표시 타임존 설정, 설정 백업/복원, 로그인 세션 관리, 업스트림 DNS strict mode/allowlist/preset, 로그인 방어 예외 정책, Cloudflare Turnstile 로그인 보호(off/always/risk-based), 보안 웹훅/이메일 알림, Slack/Discord/Telegram/Teams/PagerDuty/Email 알림 preset |
...
- 인증서 상태 체크는 `certificate_warning` / `certificate_error` 이벤트를 기록하고, 같은 상태의 중복 알림은 억제합니다.
