...
| 운영 | Docker 컨테이너 자동 감지, Cloudflare DNS 설정 UI/멀티존 드리프트 진단/일괄 재동기화/고아 레코드 정리, 시간 표시 타임존 설정, 설정 백업/복원, 로그인 세션 관리, 업스트림 DNS strict mode/allowlist/preset, 로그인 방어 예외 정책, Cloudflare Turnstile 로그인 보호(off/always/risk-based), 보안 웹훅/이메일 알림, Slack/Discord/Telegram/Teams/PagerDuty/Email 알림 preset |
...
- 감사 로그의 현재 검색·분류·기간·날짜 조건을 그대로 적용한 UTF-8 CSV를 내려받을 수 있습니다.
- 감사 로그는 기본 365일 보존 후 gzip JSONL로 아카이브하고 삭제하며, 설정 화면에서 30~3650일·아카이브 사용 여부·즉시 정리·파일 다운로드·중복 안전 복원을 관리할 수 있습니다.
- 대시보드는 영속 데이터 볼륨의 크기 제한 회전 요청 로그를 이용해 재배포 후에도 최근 6·12·24시간 Manager API 404·5xx 추이와 경로 검색 결과를 유지하고, 현재 보관 소스·용량 사용량·파일 수·회전 파일 수를 함께 표시합니다. 영속 로그 사용량이 80% 이상이거나 Docker 로그로 폴백하면 표본 손실 가능성을 경고하고, 보관 상태의 경고·복구 전이를 감사 로그와 기존 Manager 상태 운영 알림에 남깁니다. 설정에서 짧은 집계 구간의 404·5xx 임계치 알림과 제외 경로를 관리하고, 최대 최근 24시간 최고치 기반 권장값·실제 로그 관측 시작·표본 충족률·제외 경로별 오류 건수와 최근 오류 시각을 저장 전에 확인할 수 있습니다. 24시간 표본이 충족되면 현재 임계치와 권장 임계치의 상향·하향 차이를 함께 표시합니다. 상단 API 경고와 로그 보관 경고는 관련 감사 로그로 이동해 최신 이벤트 상세를 자동으로 펼칩니다.
...
- Cloudflare DNS 자동 연동은 여러 zone을 동시에 저장할 수 있고, 서비스 도메인 suffix와 가장 구체적으로 일치하는 zone만 선택해 DNS 작업을 수행합니다.
- Cloudflare를 사용하지 않는 도메인은 DNS 자동 연동, 드리프트 진단, 재동기화 대상에서 자동 제외되며, 설정 화면 결과에 제외 사유가 함께 표시됩니다.
