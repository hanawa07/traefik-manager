...
| 운영 | Docker 컨테이너 자동 감지, Cloudflare DNS 설정 UI, 시간 표시 타임존 설정, 설정 백업/복원, 로그인 세션 관리, 업스트림 DNS strict mode/allowlist/preset, 로그인 방어 예외 정책 |
| 모니터링 | 업스트림 헬스 체크, Traefik 라우터 상태, 감사 로그, 대시보드 보안 경고 요약 |
