...
| 운영 | Docker 컨테이너 자동 감지, Cloudflare DNS 설정 UI, 시간 표시 타임존 설정, 설정 백업/복원, 로그인 세션 관리, 업스트림 DNS strict mode/allowlist/preset, 로그인 방어 예외 정책, 보안 웹훅 알림 |
| 모니터링 | 업스트림 헬스 체크, Traefik 라우터 상태, 감사 로그, 대시보드 보안 경고 요약, 보안 이벤트 웹훅 전송 |
...
- 설정 화면에서 보안 웹훅 알림을 켜면 `login_locked`, `login_suspicious`, `login_blocked_ip` 이벤트를 외부 endpoint로 JSON POST 전송할 수 있습니다.
