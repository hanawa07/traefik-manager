...
- 대시보드는 영속 데이터 볼륨의 크기 제한 회전 요청 로그를 이용해 재배포 후에도 최근 6·12·24시간 Manager API 404·5xx 추이와 경로 검색 결과를 유지합니다. 영속 로그를 읽지 못하면 기존 Docker 로그로 폴백합니다. 설정에서 짧은 집계 구간의 404·5xx 임계치 알림과 제외 경로를 관리하고, 최대 최근 24시간 최고치 기반 권장값·실제 로그 관측 시작·표본 충족률·제외 경로별 오류 건수와 최근 오류 시각을 저장 전에 확인할 수 있습니다. 상단 API 경고는 관련 감사 로그로 이동해 최신 오류 이벤트 상세를 자동으로 펼칩니다.
