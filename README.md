...
| 미들웨어 | IP 허용 목록, Rate Limiting, 커스텀 응답 헤더, 미들웨어 템플릿 재사용 |
| 리다이렉트 | 도메인 간 영구/임시 리다이렉트 |
| 운영 | Docker 컨테이너 자동 감지, Cloudflare DNS 설정 UI, 설정 백업/복원 |
...
배포 시 네트워크, Traefik 연동, 검증 체크리스트는 [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md)를 참고하세요.
