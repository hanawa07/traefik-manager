...
| 운영 | Docker 컨테이너 자동 감지, Cloudflare DNS 설정 UI/일괄 재동기화/고아 레코드 정리, 시간 표시 타임존 설정, 설정 백업/복원, 로그인 세션 관리, 업스트림 DNS strict mode/allowlist/preset, 로그인 방어 예외 정책, Cloudflare Turnstile 로그인 보호(off/always/risk-based), 보안 웹훅/이메일 알림, Slack/Discord/Telegram/Teams/PagerDuty/Email 알림 preset |
