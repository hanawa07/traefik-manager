...
| 운영 | Docker 컨테이너 자동 감지, Cloudflare DNS 설정 UI, 시간 표시 타임존 설정, 설정 백업/복원, 로그인 세션 관리, 업스트림 DNS strict mode/allowlist/preset, 로그인 방어 예외 정책, Cloudflare Turnstile 로그인 보호, 보안 웹훅 알림, Slack/Discord/Telegram/Teams/PagerDuty 알림 preset |
| 모니터링 | 업스트림 헬스 체크, Traefik 라우터 상태, 감사 로그, 대시보드 보안 경고 요약, provider별 보안 이벤트 전송 |
...
- 설정 화면에서 선택형 `Cloudflare Turnstile` 로그인 보호를 켤 수 있습니다.
- Turnstile 활성화 시 로그인 페이지는 site key를 읽어 위젯을 표시하고, 백엔드는 secret key로 토큰을 검증합니다.
...
- 설정 화면에서 보안 알림 채널을 `Generic Webhook`, `Slack`, `Discord`, `Telegram`, `Microsoft Teams`, `PagerDuty` 중에서 선택할 수 있습니다.
- `login_locked`, `login_suspicious`, `login_blocked_ip` 이벤트를 채널별 포맷으로 전송합니다.
- Telegram은 bot token + chat id를 사용하고, PagerDuty는 routing key를 사용하며, Generic/Slack/Discord/Teams는 webhook URL을 사용합니다.
