...
function createDefaultCertificateDiagnosticsForm(): CertificateDiagnosticsSettingsInput {
  return {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          <SettingsCardHeader
            ...
            canEdit={canManage && !isEditingCertificateDiagnostics && !isCertificateDiagnosticsLoading}
            onEdit={handleEditCertificateDiagnostics}
          />

          {isCertificateDiagnosticsLoading ? (
            <div className="h-24 bg-gray-100 rounded-lg animate-pulse" />
          ) : isEditingCertificateDiagnostics ? (
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="label">자동 재검사 주기 (분)</label>
                  <input
                    type="number"
                    min={5}
                    max={1440}
                    className="input"
                    value={certificateDiagnosticsForm.auto_check_interval_minutes}
                    onChange={(e) =>
                      setCertificateDiagnosticsForm((current) => ({
                        ...current,
                        auto_check_interval_minutes: Number(e.target.value || 5),
                      }))
                    }
                  />
                  <p className="mt-1 text-xs text-gray-500">warning, error, pending 상태 인증서를 자동으로 다시 진단하는 간격입니다.</p>
                </div>
                <div>
                  <label className="label">반복 실패 감지 기준 (회)</label>
                  <input
                    type="number"
                    min={2}
                    max={20}
                    className="input"
                    value={certificateDiagnosticsForm.repeat_alert_threshold}
                    onChange={(e) =>
                      setCertificateDiagnosticsForm((current) => ({
                        ...current,
                        repeat_alert_threshold: Number(e.target.value || 2),
                      }))
                    }
                  />
                  <p className="mt-1 text-xs text-gray-500">같은 실패 유형이 몇 번 연속 누적되면 반복 실패로 볼지 정합니다.</p>
                </div>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label className="label">반복 실패 추적 창 (분)</label>
                  <input
                    type="number"
                    min={5}
                    max={10080}
                    className="input"
                    value={certificateDiagnosticsForm.repeat_alert_window_minutes}
                    onChange={(e) =>
                      setCertificateDiagnosticsForm((current) => ({
                        ...current,
                        repeat_alert_window_minutes: Number(e.target.value || 5),
                      }))
                    }
                  />
                  <p className="mt-1 text-xs text-gray-500">이 시간 창 안에서 같은 실패 유형이 연속되면 streak를 유지합니다.</p>
                </div>
                <div>
                  <label className="label">반복 실패 알림 쿨다운 (분)</label>
                  <input
                    type="number"
                    min={5}
                    max={10080}
                    className="input"
                    value={certificateDiagnosticsForm.repeat_alert_cooldown_minutes}
                    onChange={(e) =>
                      setCertificateDiagnosticsForm((current) => ({
                        ...current,
                        repeat_alert_cooldown_minutes: Number(e.target.value || 5),
                      }))
                    }
                  />
                  <p className="mt-1 text-xs text-gray-500">같은 실패가 반복돼도 이 시간 안에는 추가 알림을 억제합니다.</p>
                </div>
              </div>
              ...
              <SettingsActionRow>
                <button
                  className="btn-primary flex items-center gap-1.5 py-1.5 text-xs"
                  onClick={handleSaveCertificateDiagnostics}
                  disabled={updateCertificateDiagnostics.isPending}
                >
                  <Save className="w-3.5 h-3.5" />
                  {updateCertificateDiagnostics.isPending ? "저장 중..." : "저장"}
                </button>
                <button
                  className="btn-secondary flex items-center gap-1.5 py-1.5 text-xs"
                  onClick={() => setIsEditingCertificateDiagnostics(false)}
                >
                  <X className="w-3.5 h-3.5" /> 취소
                </button>
              </SettingsActionRow>
            ...
            <SettingsSummary>
              ...
              <SettingsSummaryRow
                label="반복 실패 감지 기준"
                value={`${certificateDiagnosticsSettings?.repeat_alert_threshold ?? 3}회`}
              />
              <SettingsSummaryRow
                label="반복 실패 추적 창"
