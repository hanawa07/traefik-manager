...
export function RedirectDeleteModal({
  ...
  return (
    <Modal isOpen={canManage && !!deleteTarget} onClose={onClose} title="리다이렉트 삭제">
      <p className="mb-1 text-sm text-gray-600">다음 리다이렉트를 삭제합니다:</p>
      <p className="mb-1 font-semibold text-gray-900">{deleteTarget?.domain}</p>
      <p className="mb-4 text-sm text-gray-500">→ {deleteTarget?.target_url}</p>
