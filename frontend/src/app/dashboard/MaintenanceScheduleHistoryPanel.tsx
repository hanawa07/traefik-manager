...
export function MaintenanceScheduleHistoryPanel({
  ...
  if (query.isError) {
    return <p className="text-xs text-rose-700 dark:text-rose-300">변경 이력을 불러오지 못했습니다.</p>;
  }
  if (!logs.length) {
    return <p className="text-xs text-slate-500 dark:text-slate-400">기록된 종료 시각 변경이 없습니다.</p>;
  }
