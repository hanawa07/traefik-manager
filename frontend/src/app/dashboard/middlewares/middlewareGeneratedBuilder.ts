import type { Service } from "@/features/services/api/serviceApi";
import type { TraefikMiddlewareItem } from "@/features/traefik/api/traefikApi";

import type { GeneratedMiddlewareItem } from "./middlewareGeneratedHelpers";
import { toGeneratedMiddlewareSafeName } from "./middlewareGeneratedNames";
import { formatRuntimeStatusLabel, mapRuntimeStatus } from "./middlewareRuntimeStatus";

type GeneratedMiddlewareScope = NonNullable<GeneratedMiddlewareItem["scope"]>;

export function buildGeneratedMiddlewareItems(
  service: Service,
  runtimeMap: Map<string, TraefikMiddlewareItem>,
  runtimeConnected: boolean,
): GeneratedMiddlewareItem[] {
  const routerName = toGeneratedMiddlewareSafeName(service.domain);
  const items: GeneratedMiddlewareItem[] = [];

  const register = (
    runtimeName: string,
    label: string,
    description: string,
    scope: GeneratedMiddlewareScope = "service",
  ) => {
    const runtime = runtimeMap.get(runtimeName);
    items.push({
      label,
      runtimeName,
      description,
      scope,
      status: mapRuntimeStatus(runtime, { runtimeConnected }),
      runtimeStatusLabel: formatRuntimeStatusLabel(runtime, runtimeConnected),
    });
  };

  registerAccessMiddlewares(service, routerName, register);
  registerSecurityMiddlewares(service, routerName, register);
  registerAuthMiddlewares(service, routerName, register);

  return items;
}

function registerAccessMiddlewares(
  service: Service,
  routerName: string,
  register: RegisterGeneratedMiddleware,
) {
  if (service.allowed_ips.length > 0) {
    register(
      `${routerName}-ipallowlist@file`,
      "허용 IP 미들웨어",
      `${service.allowed_ips.length}개 IP/CIDR 대역만 통과`,
    );
  }

  if (
    service.rate_limit_enabled &&
    service.rate_limit_average != null &&
    service.rate_limit_burst != null
  ) {
    register(
      `${routerName}-ratelimit@file`,
      "서비스 Rate Limit",
      `average ${service.rate_limit_average} / burst ${service.rate_limit_burst}`,
    );
  }
}

function registerSecurityMiddlewares(
  service: Service,
  routerName: string,
  register: RegisterGeneratedMiddleware,
) {
  if (Object.keys(service.custom_headers).length > 0) {
    register(
      `${routerName}-response-headers@file`,
      "응답 헤더 미들웨어",
      `${Object.keys(service.custom_headers).length}개 응답 헤더 주입`,
    );
  }

  if (service.frame_policy !== "off") {
    register(
      `${routerName}-frame-policy@file`,
      "프레임 정책",
      service.frame_policy === "sameorigin" ? "SAMEORIGIN 적용" : "DENY 적용",
    );
  }

  if (service.blocked_paths.length > 0) {
    register(
      `${routerName}-block@file`,
      "차단 경로 보호",
      `${service.blocked_paths.length}개 경로 차단`,
    );
  }

  if (service.tls_enabled && service.https_redirect_enabled) {
    register(
      `${routerName}-redirectscheme@file`,
      "HTTP → HTTPS 리다이렉트",
      "HTTP 요청을 HTTPS로 강제 전환",
    );
  }
}

function registerAuthMiddlewares(
  service: Service,
  routerName: string,
  register: RegisterGeneratedMiddleware,
) {
  if (service.basic_auth_enabled) {
    register(
      `${routerName}-basicauth@file`,
      "서비스 Basic Auth",
      `${service.basic_auth_user_count}개 계정 인증`,
    );
  }

  if (service.auth_mode === "token") {
    register(
      `${routerName}-token-auth@file`,
      "백엔드 토큰 ForwardAuth",
      "서비스 전용 API Key 검증",
    );
  }

  if (service.auth_mode === "authentik") {
    register(
      "authentik@file",
      "Authentik ForwardAuth",
      "공용 Authentik 미들웨어를 이 서비스가 공유 사용",
      "shared",
    );
  }
}

type RegisterGeneratedMiddleware = (
  runtimeName: string,
  label: string,
  description: string,
  scope?: GeneratedMiddlewareScope,
) => void;
