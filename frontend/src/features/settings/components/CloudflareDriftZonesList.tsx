  ...
  return (
    <div className="grid gap-3 xl:grid-cols-2">
      {zones.map((zone) => (
        <div
          key={zone.zone_name}
          className="rounded-lg border border-white/60 bg-white/70 p-3 text-xs text-gray-700 dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-300"
        >
          <div className="flex items-center justify-between gap-3">
            <p className="font-medium text-gray-900 dark:text-slate-100">{zone.zone_name}</p>
...
function CloudflareDriftZoneStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-md border border-gray-200 bg-white p-2 dark:border-slate-700 dark:bg-slate-950">
      <p className="text-gray-500 dark:text-slate-400">{label}</p>
      <p className="mt-1 font-medium text-gray-900 dark:text-slate-100">{value}개</p>
    </div>
  );
}
