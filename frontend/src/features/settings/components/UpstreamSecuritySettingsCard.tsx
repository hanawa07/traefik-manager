      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
              ...
              {presets.map((preset) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      onFormChange(applyUpstreamPreset(formValue, preset))
                    }
                  >
          ...
          <CheckboxRow
            ...
            description={
              "도메인 업스트림 저장 시 DNS를 다시 조회해서 loopback, link-local, " +
              "문서 예제 대역 같은 금지 주소로 해석되는지 검사합니다."
            }
            onChange={(checked) =>
              onFormChange({ ...formValue, dns_strict_mode: checked })
            }
          />
          ...
          <CheckboxRow
            ...
            description={
              "외부 FQDN은 아래 suffix 목록과 일치해야만 저장할 수 있습니다. " +
              "strict mode와는 별개로 동작합니다."
            }
            onChange={(checked) =>
              onFormChange({ ...formValue, allowlist_enabled: checked })
            }
          />
          ...
          <CheckboxRow
            ...
            onChange={(checked) =>
              onFormChange({ ...formValue, allow_docker_service_names: checked })
            }
          />
          ...
          <CheckboxRow
            ...
            description={
              ...
              "`100.64.0.0/10` 대역 IP 리터럴을 허용합니다."
            }
            onChange={(checked) =>
              onFormChange({ ...formValue, allow_private_networks: checked })
            }
          />
        ...
        <SettingsSummary>
          ...
          <SettingsSummaryRow
            ...
            value={settings?.dns_strict_mode ? "활성화" : "비활성화"}
          />
          <SettingsSummaryRow
            label="업스트림 allowlist"
            value={settings?.allowlist_enabled ? "활성화" : "비활성화"}
          />
          <SettingsSummaryRow
            label="허용 suffix"
            value={
              settings?.allowed_domain_suffixes?.length
                ? `${settings.allowed_domain_suffixes.length}개`
                : "없음"
            }
          />
          <SettingsSummaryRow
            label="Docker 서비스명"
            value={settings?.allow_docker_service_names ? "허용" : "차단"}
          />
          <SettingsSummaryRow
