  ...
  return (
    <div className="card p-6 h-full order-4">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(event) =>
                updateForm({ suspicious_trusted_networks_text: event.target.value })
              }
            />
          ...
          <LoginDefenseCheckboxRow
            ...
            onChange={(checked) =>
              updateForm({ suspicious_block_escalation_enabled: checked })
            }
          />
                ...
                onChange={(event) =>
                  updateForm({
                    suspicious_block_max_minutes: Number(
                      event.target.value || (settings?.suspicious_block_minutes ?? 30),
                    ),
                  })
                }
              />
          ...
          <p className="text-xs text-gray-500 -mt-2">
            기본 차단 시간은 {formatDurationMinutes(settings?.suspicious_block_minutes ?? 30)}이며,
            반복 차단 시 배수만큼 늘어나되 최대 시간에서 멈춥니다.
          </p>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  updateForm({ turnstile_site_key: event.target.value })
                }
              />
              <p className="mt-1 text-xs text-gray-500">
                로그인 페이지에 공개로 노출되는 site key입니다.
              </p>
            </div>
            <div>
              ...
              <input
                ...
                onChange={(event) =>
                  updateForm({ turnstile_secret_key: event.target.value })
                }
              />
              <p className="mt-1 text-xs text-gray-500">
                {settings?.turnstile_secret_key_configured
        ...
        <LoginDefenseSummary settings={settings} />
      )}
    </div>
  );
}
...
function LoginDefensePolicySummary({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600 space-y-1">
      <p>
        계정 잠금 정책: {formatDurationMinutes(settings?.failure_window_minutes)} 동안{" "}
      ...
      <p>
        ...
        {settings?.suspicious_failure_count}회 실패 + 서로 다른 사용자명{" "}
        {settings?.suspicious_username_count}개 이상
      </p>
...
function LoginDefenseSummary({
  ...
  return (
    <SettingsSummary>
      ...
      <SettingsSummaryRow
        label="추가 로그인 검증"
        value={getTurnstileModeLabel(settings?.turnstile_mode ?? "off")}
      />
      {settings?.turnstile_mode !== "off" ? (
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600 space-y-1">
          ...
          <p>
            Secret Key:{" "}
            {settings?.turnstile_secret_key_configured ? "설정됨" : "(미설정)"}
          </p>
        </div>
      ) : null}
      ...
      <p className="text-xs text-gray-500">
        신뢰 네트워크 예외는 내부 NAT, VPN, 사내망처럼 반복 실패가 운영 노이즈로 잡힐 수 있는
        경로에만 제한적으로 사용하세요.
      </p>
    </SettingsSummary>
  );
