  ...
  return (
    <span
      className={clsx(
        ...
        tlsEnabled ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500",
      )}
    >
...
function AuthBadge({ service }: { service: Service }) {
  if (service.auth_mode === "authentik") {
    return (
      <span className={authBadgeClassName("bg-blue-100 text-blue-700")}>
  ...
  if (service.auth_mode === "token") {
    return (
      <span className={authBadgeClassName("border border-purple-200 bg-purple-100 text-purple-700")}>
  ...
  return (
    <span className={authBadgeClassName("bg-gray-100 text-gray-500")}>
      <Lock className="h-3 w-3" />
      인증 없음
    </span>
  );
