        ...
        {filteredTraefikImportCandidates.length === 0 ? (
          ...
          filteredTraefikImportCandidates.map((candidate) => (
            <button
              ...
              type="button"
              className="w-full rounded-xl border border-gray-200 bg-white p-4 text-left transition-colors hover:border-indigo-300 hover:bg-indigo-50 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-indigo-500 dark:hover:bg-indigo-950/30"
              onClick={() => onImport(candidate)}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-gray-900 dark:text-slate-100">{candidate.domain}</p>
                ...
                <span
                  className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${
                    candidate.tls_enabled ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-200" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300"
                  }`}
                >
