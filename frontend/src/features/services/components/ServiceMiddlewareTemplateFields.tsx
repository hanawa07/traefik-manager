...
export default function ServiceMiddlewareTemplateFields({
  ...
  return (
    <div>
      ...
      {isMiddlewareLoading ? (
        <div className="h-20 animate-pulse rounded-lg bg-gray-50 dark:bg-slate-800" />
      ) : middlewareTemplates.length === 0 ? (
        <p className="rounded-lg border bg-gray-50 p-3 text-xs text-gray-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400">
          등록된 템플릿이 없습니다.
        </p>
      ) : (
        <div className="max-h-48 space-y-2 overflow-y-auto rounded-lg border border-gray-200 p-3 dark:border-slate-700">
