      ...
      <div className="space-y-3 pl-1">
        <label className="flex cursor-pointer items-start gap-3">
          ...
          <div className="min-w-0">
            <span className="text-sm font-medium text-gray-700 dark:text-slate-200">HTTPS (TLS) 활성화</span>
            <p className="text-xs text-gray-500 dark:text-slate-400">Let&apos;s Encrypt 인증서 자동 발급</p>
          </div>
        </label>
        ...
        <label className={`flex items-start gap-3 ${tlsEnabled ? "cursor-pointer" : "cursor-not-allowed opacity-60"}`}>
          ...
          />
          <div className="min-w-0">
            <span className="text-sm font-medium text-gray-700 dark:text-slate-200">HTTP → HTTPS 자동 리다이렉트</span>
            <p className="text-xs text-gray-500 dark:text-slate-400">HTTP 요청을 HTTPS로 강제 전환합니다</p>
          </div>
        </label>
        ...
        <div className="pt-1">
          <label className="flex cursor-pointer items-start gap-3">
            ...
            />
            <div className="min-w-0">
              <span className="text-sm font-medium text-gray-700 dark:text-slate-200">업스트림 HTTPS 사용</span>
              <p className="text-xs text-gray-500 dark:text-slate-400">백엔드 서비스가 자체 HTTPS를 사용하는 경우 체크</p>
            </div>
          </label>
          ...
          {upstreamScheme === "https" && (
            <label className="mt-3 ml-7 flex cursor-pointer items-start gap-3 animate-in fade-in slide-in-from-left-2">
              ...
              />
              <div className="min-w-0">
                <span className="text-sm font-medium text-gray-700 dark:text-slate-200">TLS 인증서 검증 무시</span>
                <p className="text-xs text-gray-500 dark:text-slate-400">자체서명 인증서를 사용하는 경우 체크</p>
              </div>
            </label>
          )}
        </div>
