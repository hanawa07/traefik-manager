  ...
  if (upstreamHealth?.status === "down" && upstreamHealth.error) {
    return (
      <div className="mt-2 space-y-1 break-words text-[11px] leading-4">
        {certificate ? <p className="text-slate-500">인증서 상태: {certificate.status_message}</p> : null}
        ...
        <p className="text-slate-500">확인 시각: {formatDateTime(upstreamHealth.checked_at, displayTimeZone)}</p>
        {lastSuccessAt ? <p className="text-slate-500">최근 성공: {formatDateTime(lastSuccessAt, displayTimeZone)}</p> : null}
        <p className="truncate text-slate-400" title={upstreamHealth.checked_url}>
  ...
  if (upstreamHealth?.status === "up") {
    return (
      <div className="mt-2 space-y-1 break-words text-[11px] leading-4">
        ...
        <p className="text-slate-500">확인 시각: {formatDateTime(upstreamHealth.checked_at, displayTimeZone)}</p>
        {lastFailureAt ? <p className="text-slate-500">최근 실패: {formatDateTime(lastFailureAt, displayTimeZone)}</p> : null}
        <p className="truncate text-slate-400" title={upstreamHealth.checked_url}>
          체크 URL: {upstreamHealth.checked_url}
        </p>
      </div>
