    ...
    <div className="grid grid-cols-2 gap-2 rounded-xl bg-gray-100 p-1 dark:bg-slate-800">
      <button
        ...
        className={`rounded-lg px-3 py-2 text-left text-sm font-medium transition-colors ${
          mode === "basic" ? "bg-white text-gray-900 shadow-sm dark:bg-slate-950 dark:text-slate-100" : "text-gray-600 hover:text-gray-900 dark:text-slate-400 dark:hover:text-slate-100"
        }`}
        onClick={() => onModeChange("basic")}
      >
        <span className="flex items-center justify-between gap-2">
          일반 컨테이너
          <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600 dark:bg-slate-800 dark:text-slate-300">{basicCount}</span>
        </span>
      ...
      <button
        type="button"
        className={`rounded-lg px-3 py-2 text-left text-sm font-medium transition-colors ${
          mode === "traefik" ? "bg-white text-gray-900 shadow-sm dark:bg-slate-950 dark:text-slate-100" : "text-gray-600 hover:text-gray-900 dark:text-slate-400 dark:hover:text-slate-100"
        }`}
